...
// GetTCPAuthFrame returns the header payload for the TCP connection
//
// connectionId: the connection ID to use in the header
//
// clientId: the client ID to use in the header
//
// Example: GetTCPAuthFrame("connection-id", 123)
func GetTCPAuthFrame(connectionId string, clientId int) []byte {
	...
	frame = append(frame, []byte(connectionId)...)
	frame = append(frame,
